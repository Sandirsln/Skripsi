...
void runBenchmark() {
    ...
    // Header pembuka ini tidak masalah, skrip Python akan mengabaikannya
    Serial.println("Starting benchmark for FLOAT model (Case A)...");
    Serial.println("--------------------------------------------------");

    // (PERBAIKAN FINAL) Header tabel disesuaikan 100% dengan yang dicari skrip Python.
    // "Sample" diubah menjadi "Input" dan "Latency (us)" menjadi "Time (us)"
    ...
    for (int i = 0; i < TEST_SAMPLES_COUNT_A; i++) {
        ...
        // DIUBAH: kolom pertama sekarang adalah 'input', bukan 'sample index (i)'
        // agar sesuai dengan header "Input".
...
void setup() {
    ...
    Serial.println("--- TFLite Float Model Benchmark (Case A, MSE) for RP2350/RP2040 ---");
